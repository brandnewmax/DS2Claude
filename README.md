...
```bash
curl -fsSL https://raw.githubusercontent.com/brandnewmax/DS2Claude/main/scripts/install.sh | bash
```

或者手动：

```bash
git clone https://github.com/brandnewmax/DS2Claude.git
cd DS2Claude
...
```bash
# 1. 编辑 groups.yaml，填入你的 API Key
#    （文件里有注释，每个平台的格式照抄即可）

# 2. 启动代理
python3 proxy.py

# 3. 浏览器打开 http://127.0.0.1:8765 可视化管理

# 4. Claude Desktop → 设置 → 开发者模式 → Gateway
#    Base URL: http://127.0.0.1:8765
#    API Key:  随便填（代理会自动替换为真实 Key）
#    Models:   claude-opus-4-7, claude-sonnet-4-6, claude-haiku-4-5
```

> 模型名 `claude-opus-4-7` / `claude-sonnet-4-6` / `claude-haiku-4-5` 会按照 `groups.yaml` 中**当前激活组**的映射，替换为真实模型名后转发到对应平台。
